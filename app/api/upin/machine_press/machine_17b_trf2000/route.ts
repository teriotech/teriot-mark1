  ...
  try {
    ...
    if (id) {
      const [rows]: any = await db.execute(
        'SELECT * FROM 17b_fg_machine_trf2000 WHERE id = ?',
        [id]
      );
    ...
    const [rows] = await db.execute(
      'SELECT * FROM 17b_fg_machine_trf2000 ORDER BY id DESC'
    );
 ...
 * URL Query: /api/machine_press/machine_17b_trf2000?id=1
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const [result]: any = await db.execute(
      'DELETE FROM 17b_fg_machine_trf2000 WHERE id = ?',
      [id]
    );
