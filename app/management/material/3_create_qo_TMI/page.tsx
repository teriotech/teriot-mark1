      ...
      <div className="min-h-screen bg-slate-900 text-slate-100 p-6 font-sans print:hidden">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-white flex items-center gap-3"><Layers className="w-8 h-8 text-blue-500" /> Quotation, PO, BAST & Invoice Generator</h1>
              <p className="text-sm text-slate-400 mt-1">Generate dokumen QO / PO / BAST / Invoice per Customer berdasarkan data BOM.</p>
            </div>
            <button onClick={() => setIsCreateOpen(true)} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-medium shadow-lg shadow-blue-500/20 transition-all duration-200"><Plus className="w-5 h-5" /> Create New</button>
          </div>
              ...
              {filteredBomGroups.map((group) => {
                ...
                return (
                  <div key={group.customer} className="bg-slate-800/60 border border-slate-700/60 rounded-xl overflow-hidden transition-all duration-200">
                    <div onClick={() => toggleCustomerExpand(group.customer)} className="p-5 flex flex-col md:flex-row md:items-center justify-between cursor-pointer hover:bg-slate-700/30 transition-colors gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="text-lg font-bold text-white flex items-center gap-2">
                            {group.customer}
                            {group.qo_number && (
                              <span className="ml-2 px-2 py-0.5 text-[10px] font-semibold bg-blue-500/20 text-blue-300 rounded-full border border-blue-500/30">
                                {group.qo_number}
                              </span>
                            )}
                          </h3>
                      ...
                      <div className="flex items-center gap-4 md:gap-6">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={(e) => { e.stopPropagation(); setPrintConfig({ group, type: "QO" }); }} className="p-2 text-slate-400 hover:text-blue-400 hover:bg-slate-700/50 rounded-lg transition-colors" title="Generate Quotation (QO)"><FileText className="w-5 h-5" /></button>
                          <button onClick={(e) => { e.stopPropagation(); setPrintConfig({ group, type: "PO" }); }} className="p-2 text-slate-400 hover:text-emerald-400 hover:bg-slate-700/50 rounded-lg transition-colors" title="Generate Purchase Order (PO)"><ShoppingCart className="w-5 h-5" /></button>
                          <button onClick={(e) => { e.stopPropagation(); setBastConfig({ group }); }} className="p-2 text-slate-400 hover:text-purple-400 hover:bg-slate-700/50 rounded-lg transition-colors" title="Generate BAST"><ClipboardCheck className="w-5 h-5" /></button>
                          <button onClick={(e) => { e.stopPropagation(); setInvoiceConfig({ group }); }} className="p-2 text-slate-400 hover:text-amber-400 hover:bg-slate-700/50 rounded-lg transition-colors" title="Generate Invoice"><Receipt className="w-5 h-5" /></button>
                                    ...
                                    {sortedChildren.map((child) => {
                                      ...
                                      return (
                                        <tr key={child.id} className="hover:bg-slate-700/20">
                                          ...
                                          <td className="px-4 py-2.5 text-right font-medium text-emerald-400">Rp {totalPrice.toLocaleString("id-ID")}</td>
                                        </tr>
                                      );
                                    })}
                                  </tbody>
